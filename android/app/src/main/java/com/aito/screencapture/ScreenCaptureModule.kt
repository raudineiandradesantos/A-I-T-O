            ...
            when (resultCode) {
                Activity.RESULT_OK -> {
                    if (data != null) {
                        ...
                        Log.d(TAG, "Ã¢ÂÂ MediaProjection permission granted (Android 14+ app selection: $isAndroid14Plus)")
                        
                        pendingPromise?.resolve(true)
                    } else {
                        Log.e(TAG, "Ã¢ÂÂ Result OK but data is null")
                        pendingPromise?.reject("ERROR", "Permission granted but data is null")
                    }
                }
                Activity.RESULT_CANCELED -> {
                    Log.w(TAG, "Ã¢ÂÂ User canceled screen capture permission")
                    pendingPromise?.reject("PERMISSION_DENIED", "User canceled screen capture permission")
                }
                else -> {
                    Log.e(TAG, "Ã¢ÂÂ Unexpected result code: $resultCode")
